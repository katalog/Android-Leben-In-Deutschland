...
- [ ] 접근성 점검 (색상 대비, 폰트 크기 등) — 색상은 디자인 스펙(accent700 규칙 등)을 그대로 따르고 있어 기본은 지켜지고 있으나, 별도 접근성 감사(TalkBack 등)는 아직 안 함
...
## 다음 할 일 (Next Up)
1. **핵심 기능 5가지(문제 데이터, 모의고사, 오답 재연습, 통계, 온디바이스 번역) 전부 구현 완료.** RTL도 완료. 남은 Phase 7 항목: 아이콘 세트, 접근성 점검
2. Phase 8(출시 준비: 앱 아이콘, 스토어 자산, 서명, 테스트)은 아직 시작 전
3. 알려진 제약: 시험 도중 앱 프로세스가 완전히 강제종료되면 타이머/진행상황이 복구되지 않음 (화면 회전은 문제없음)
4. Bundesland는 한 번 선택하면 바꿀 UI가 없음 (Mehr 화면에 재선택 진입점 추가 고려)
5. 언어 재선택 시 네비게이션 백스택이 완전히 정리되지 않는 사소한 이슈
6. 번역 텍스트가 길 경우 레이아웃 줄바꿈/여러 줄 처리는 육안 확인만 했고 극단적으로 긴 문자열(예: 우크라이나어 등)에 대한 별도 검증은 아직 안 함
